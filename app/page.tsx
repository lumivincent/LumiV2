...
function AiWorkspaceModal({ mode, workspace, initialTitle, initialPrompt = '', initialKnowledgePaths = [], initialSourceIds = [], onClose, onSaved, refresh, setNotice }: { mode: 'knowledge' | 'analysis'; workspace: Workspace; initialTitle: string; initialPrompt?: string; initialKnowledgePaths?: string[]; initialSourceIds?: string[]; onClose: () => void; onSaved?: (path: string) => void; refresh: (silent?: boolean) => Promise<void>; setNotice: (notice: Notice) => void }) {
  ...
  useEffect(() => {
    fetch('/api/assistant', { cache: 'no-store' }).then(async (response) => await response.json() as { configured?: boolean; codexAvailable?: boolean }).then((result) => { setApiConfigured(Boolean(result.configured)); setCodexAvailable(result.codexAvailable !== false); }).catch(() => setApiConfigured(false));
  }, []);

  useEffect(() => { if (!codexAvailable) setProvider('api'); }, [codexAvailable]);
  ...
  function startNew() {
    ...
    setSessionId(''); setTitle(initialTitle); setProvider(codexAvailable ? 'codex' : 'api'); setLastProvider(undefined); setMessages([]); setKnowledgePaths(initialKnowledgePaths); setIncludeSources(mode === 'analysis'); setSourceIds(initialSourceIds); setApiResponseId(''); setCodexThreadId(''); setInput(initialPrompt); setCodexRun(null); setErrorText(''); setTab('chat');
  }

  function resumeLatest() {
    if (!latestSession) return;
    setSessionId(latestSession.id); setTitle(latestSession.title); setProvider(!codexAvailable && latestSession.provider === 'codex' ? 'api' : latestSession.provider); setLastProvider(latestSession.lastProvider); setMessages(latestSession.messages); setKnowledgePaths(latestSession.knowledgePaths); setIncludeSources(latestSession.includeSources); setSourceIds(latestSession.sourceIds ?? []); setApiResponseId(latestSession.apiResponseId ?? ''); setCodexThreadId(latestSession.codexThreadId ?? ''); setInput(''); setTab('chat'); setErrorText('');
  }
  ...
  return <div className="ai-modal-backdrop" onMouseDown={() => !working && onClose()}><section className="ai-workspace-modal" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {tab === 'direct' ? <div className="knowledge-direct-form"><div className="direct-type-row">{([['source', '源头资料'], ['insight', '结论'], ['topic', '主题'], ['discussion', '讨论记录']] as const).map(([value, label]) => <button className={directType === value ? 'active' : ''} onClick={() => setDirectType(value)} type="button" key={value}>{label}</button>)}</div><div className="direct-field-grid"><label>标题（可选）<input value={directTitle} onChange={(event) => setDirectTitle(event.target.value)} placeholder="不填时自动取正文首行" /></label><label>标签（可选）<input value={directTags} onChange={(event) => setDirectTags(event.target.value)} placeholder="用逗号或空格分隔" /></label></div>{directType === 'source' && <label>原始链接（可选）<input value={directUrl} onChange={(event) => setDirectUrl(event.target.value)} placeholder="https://…" /></label>}<label>正文<textarea value={directContent} onChange={(event) => setDirectContent(event.target.value)} placeholder={directType === 'source' ? '补充摘要、摘录或为什么值得保留…' : '输入需要长期保留的内容…'} /></label><KnowledgeReferencePicker workspace={workspace} selected={knowledgePaths} onChange={setKnowledgePaths} query={`${directTitle}\n${directContent}`} label="归入主题" emptyLabel="选择主题" types={['topic']} /><div className="ai-direct-actions"><span>未保存内容会在本次浏览器会话中自动保留</span><button className="primary" onClick={saveDirect} disabled={!canSaveDirect} type="button">加入知识库</button></div>{errorText && <p className="ai-error">{errorText}</p>}</div> : <>
      <div className="ai-context-bar"><div className="ai-toolbar-group"><span>执行</span><div className="execution-switch">{codexAvailable && <button className={provider === 'codex' ? 'active' : ''} onClick={() => setProvider('codex')} type="button">Codex</button>}<button className={provider === 'api' ? 'active' : ''} onClick={() => setProvider('api')} disabled={apiConfigured === false} title={apiConfigured === false ? '尚未配置 OpenAI API' : '使用 OpenAI API'} type="button">OpenAI API</button></div></div><div className="ai-toolbar-group context"><span>本次参考</span>{mode === 'analysis' && <label className="source-toggle"><input type="checkbox" checked={includeSources} onChange={(event) => setIncludeSources(event.target.checked)} />{sourceIds.length === 1 ? workspace.sources.find((source) => source.id === sourceIds[0])?.title ?? '当前分类' : `全部 ${workspace.sources.length} 类文档`}</label>}<KnowledgeReferencePicker workspace={workspace} selected={knowledgePaths} onChange={setKnowledgePaths} query={`${title}\n${input}\n${messages.map((item) => item.content).join('\n')}`} label="知识库" /></div></div>
...
function ContentWorkbench({ workspace, refresh, setNotice, setView }: { workspace: Workspace; refresh: (silent?: boolean) => Promise<void>; setNotice: (notice: Notice) => void; setView: (view: View) => void }) {
  ...
  useEffect(() => {
    fetch('/api/content', { cache: 'no-store' })
      .then(async (response) => await response.json() as { configured?: boolean; codexAvailable?: boolean; model?: string })
      .then((result) => { setApiConfigured(Boolean(result.configured)); setCodexAvailable(result.codexAvailable !== false); if (result.model) setApiModel(result.model); })
      .catch(() => setApiConfigured(false));
  }, []);

  useEffect(() => { if (!codexAvailable) setExecutionMode('api'); }, [codexAvailable]);
    ...
    <div className="content-workbench-layout">
      ...
      <main className="content-workbench-main">
        {!isFinalized && <section className="box content-composer"><BoxHeader title="快速创作" />
          <div className="content-memory-strip"><span><i />最新产品内容 <strong>{formatTime(latestSourceUpdate)}</strong></span><span><i />定稿内容记忆 <strong>{approvedCount} 条</strong></span><span className="shared-memory-note">{codexAvailable ? '两种方式共用' : '托管版 · OpenAI API'}</span></div>
...
function AssetStudio({ workspace, refresh, setNotice, active }: { workspace: Workspace; refresh: (silent?: boolean) => Promise<void>; setNotice: (notice: Notice) => void; active: boolean }) {
  ...
  useEffect(() => { fetch('/api/image', { cache: 'no-store' }).then(async (response) => await response.json() as { configured?: boolean; codexAvailable?: boolean; model?: string }).then((result) => { setImageConfigured(Boolean(result.configured)); setCodexAvailable(result.codexAvailable !== false); if (result.model) setImageModel(result.model); }).catch(() => setImageConfigured(false)); }, []);
  useEffect(() => { if (!codexAvailable) setExecutionMode('api'); }, [codexAvailable]);
  useEffect(() => {
...
function KnowledgeBase({ workspace, refresh, setNotice }: { workspace: Workspace; refresh: (silent?: boolean) => Promise<void>; setNotice: (notice: Notice) => void }) {
  ...
  useEffect(() => {
    const restorePath = () => {
      ...
      if (item.type === 'source' && !isKnowledgeProjectReport(item)) setSourcesOpen(true);
    };
    restorePath();
  ...
  }, [reportsOpen, sourcesOpen, selectedPath]);

  function openKnowledgePath(path: string, historyMode: 'push' | 'replace' = 'push') {
    const item = workspace.knowledgeMetadata.find((entry) => entry.path === path);
  ...
  return <div className="page knowledge-page">
    ...
    <div className="knowledge-split-layout">
      <section ref={feedRef} className="box knowledge-feed knowledge-directory">
        <BoxHeader title={query ? `${searchScope === 'global' ? '全库' : '主题内'}搜索 · ${visibleItems.length}` : '知识目录'} />
        {query ? visibleItems.length === 0 ? <div className="knowledge-friendly-empty"><strong>没有找到相关内容</strong><p>换一个词试试，标题、正文和标签都可以搜索。</p></div> : <div className="knowledge-search-results">{visibleItems.map((item) => renderDirectoryItem(item))}</div> : topics.map((topic) => {
          const expanded = selectedTopic?.id === topic.id;
          const directoryItems = allKnowledgeItems.filter((item) => item.id === topic.id || item.topicIds.includes(topic.id));
          const directItems = directoryItems.filter((item) => !isKnowledgeProjectReport(item) && item.type !== 'source');
          const sourceItems = directoryItems.filter((item) => item.type === 'source' && !isKnowledgeProjectReport(item));
          const projectItems = directoryItems.filter(isKnowledgeProjectReport);
          return <section className={`knowledge-tree-topic ${expanded ? 'active' : ''}`} key={topic.id}>
            <button className="knowledge-tree-topic-button" aria-expanded={expanded} onClick={() => chooseTopic(topic)} title={topic.title} type="button"><span>{expanded ? '⌄' : '›'}</span><strong>{shortKnowledgeTopicTitle(topic.title)}</strong><small>{directoryItems.length} 项</small></button>
            {expanded && <div className="knowledge-tree-submenu">
              {directItems.map((item) => renderDirectoryItem(item))}
              {sourceItems.length > 0 && <section className="knowledge-tree-group"><button aria-expanded={sourcesOpen} onClick={() => setSourcesOpen((value) => !value)} type="button"><span>{sourcesOpen ? '⌄' : '›'}</span><strong>相关资料</strong><small>{sourceItems.length}</small></button>{sourcesOpen && <div>{sourceItems.map((item) => renderDirectoryItem(item, true))}</div>}</section>}
              {projectItems.length > 0 && <section className="knowledge-tree-group"><button aria-expanded={reportsOpen} onClick={() => setReportsOpen((value) => !value)} type="button"><span>{reportsOpen ? '⌄' : '›'}</span><strong>项目报告</strong><small>{projectItems.length}</small></button>{reportsOpen && <div>{projectItems.map((item) => renderDirectoryItem(item, true))}</div>}</section>}
            </div>}
          </section>;
        })}
      </section>
    ...
    {topicCreateOpen && <TopicCreateModal onClose={() => setTopicCreateOpen(false)} onCreated={(path) => { openKnowledgePath(path); setSearch(''); }} refresh={refresh} setNotice={setNotice} />}
  </div>;
}
